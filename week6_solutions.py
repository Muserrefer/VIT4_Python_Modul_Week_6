...
class TaskEditing:
    ...
    def add_note_to_task(self):
        task_identifier = input("Enter the name or number of the task to add a note: ")
        similar_tasks = self.find_similar_tasks(task_identifier)
        ...
        task_index = input("Enter the index of the task you want to add a note to: ")
        try:
            task_index = int(task_index)
        except ValueError:
            print("Invalid input. Please enter a valid index.")
        ...
        task_to_add_note = similar_tasks[task_index]
        note = input("Enter the note: ")
        task_to_add_note.add_note(note)
...
class TaskManagement:
    ...
    def print_pending_tasks(self):
        personal_tasks = [
            ...
            and datetime.strptime(task.deadline, "%d/%m/%Y").date() >= datetime.now().date()
        ]
        work_tasks = [
            task for task in self.tasks 
